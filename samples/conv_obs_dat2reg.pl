#!/usr/bin/perl
#
# convert obs file for glafic to ds9 region file
#
...
if((($#ARGV+1)!=1)&&(($#ARGV+1)!=2)){
    die "usage:\n > conv_obs_dat2reg.pl model_hoge.input [obs_hoge.dat]\n    [obs_hoge.dat] is optional\n";
}
...
while(<IN>){
	...
	if($com eq 'readobs_point'){ $ofile=$data[1]; }
    }
}
...
while(<IN>){
	...
	if(/[0-9]/){
		...
		if(substr($_,0,1)  ne '#'){
		    ...
		    $to=sprintf("%s.%s",$idid,$i);
		    
		    if($x0 =~ /[0-9]/){
			print OUT "circle($xs,$ys,$rr\") # width=2 text={$to} color=$clr\n";
		    } else {
			print OUT "circle($xs,$ys,$rr) # width=2 text={$to} color=$clr\n";
		    }
		} else {
